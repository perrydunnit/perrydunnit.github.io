    ...
    pub fn new(segments: js_sys::Array, speed: f64, width: usize, height: usize) -> Spirograph {
        let segments = segments
            ...
            .map(|s| {
                ...
                Segment {
                    length: js_sys::Reflect::get(&obj, &"length".into())
                        .unwrap()
                        .as_f64()
                        .unwrap(),
                    angle: js_sys::Reflect::get(&obj, &"angle".into())
                        .unwrap()
                        .as_f64()
                        .unwrap(),
                    angular_speed: js_sys::Reflect::get(&obj, &"angularSpeed".into())
                        .unwrap()
                        .as_f64()
                        .unwrap(),
                    alpha: js_sys::Reflect::get(&obj, &"alpha".into())
                        .unwrap()
                        .as_f64()
                        .unwrap(),
                    color_speed: js_sys::Reflect::get(&obj, &"colorSpeed".into())
                        .unwrap()
                        .as_f64()
                        .unwrap(),
                }
    ...
    pub fn draw_points(&mut self, steps_per_frame: u32, scale: f64) {
        ...
        for _ in 0..steps_per_frame {
            ...
            for seg in &mut self.segments {
                ...
                let raw_hue = (seg.color_speed) * self.base_angle * 180.0 / std::f64::consts::PI;
                let hue = ((raw_hue % 360.0) + 360.0) % 360.0; // Normalized to [0, 360)
                ...
                // I want 0 degrees to point up, so we need to adjust the coordinates
                ...
                let rotated_x = dy + center_x;
                let rotated_y = -dx + center_y;

                // Convert to buffer index
                let xi = rotated_x.round() as isize;
                let yi = rotated_y.round() as isize;
